...
/**
 * The World (3D) view: renders the bin + pile shapes, reports clicks as a
 * world-space ray, and animates collected shapes flying into the 3D slot rack.
 * It never reads the physics world.
 */
export interface IPileView extends IView {
  ...
  /** A pointer click, as a ray from the camera in world space. */
  onPick(cb: (ox: number, oy: number, oz: number, fx: number, fy: number, fz: number) => void): Unsubscribe;
}
