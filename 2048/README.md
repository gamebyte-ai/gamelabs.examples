...
`npm run playable:dev` serves the same single-file entry locally (on port 5301) for QA.

It reuses the regular `src/main.ts` entry — Vite's single-file build automatically inlines
both the example's own `new URL(...)` assets (here the 4 SFX) and the framework's default UI
textures, so no extra wiring is needed.
